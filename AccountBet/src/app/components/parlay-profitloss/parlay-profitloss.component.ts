...
@Component({
  selector: 'app-parlay-profitloss',
  templateUrl: './parlay-profitloss.component.html',
  styleUrls: ['./parlay-profitloss.component.scss']
})
export class ParlayProfitlossComponent implements OnInit {
  ...
  GetProfitLoss() {
    ...
    this.reportService.parlayBetsProfitLoss(pnldates.fromdate, pnldates.todate).subscribe(
      (resp: any) => {
        if (this.stype == '') {
          this.ProfitLoss = resp.result;
          this.ProfitLoss = this.ProfitLoss.map(bets => {
            if (bets.bets) {
              switch (bets.bets?.length) {
                case 1:
                ...
                case 10:
                  ...
                  break;
                default:
                  bets['countName'] = `Multiple (${bets.bets?.length})`;
              }
            }
            return bets;
          });
        ...
        if (this.ProfitLoss?.length == 0 && this.isToastr) {
